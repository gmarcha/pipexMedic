...
printf "$PURPLE""test: < test1 grep hello | awk '{count++} END {print count}' > test2$RESET\n"
printf "$PURPLE$S2$RESET\n"
< test1 grep hello | awk '{count++} END {print count}' > test2
../pipex user1 "grep hello" "awk '{count++} END {print count}'" user2
comp test2 user2
rm -rf test2 user2
